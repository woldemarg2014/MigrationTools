...
Процедура КнопкаВыполнитьНажатие(Кнопка)
	...
	Для каждого Проверка Из ТаблицаПроверок Цикл
		Если Проверка.Проверять Тогда
			...
			ИначеЕсли ВРег(СокрЛП(Проверка.ВидПроверки)) = ВРег(СокрЛП("6. Проверка корректности остатков товаров на складах")) Тогда	
				СтруктураПараметров = ПроверкаКорректностиОстатковКупленныхИПринятыхНаКомиссиюТоваров(Отчет);
				ЕстьОшибки = СтруктураПараметров.ЕстьОшибки;
			ИначеЕсли ВРег(СокрЛП(Проверка.ВидПроверки)) = ВРег(СокрЛП("7. Проверка корректности остатков заказов покупателей товаров")) Тогда	
				СтруктураПараметров = ПроверитьКорректностьОстатковЗаказовПокупателей(Отчет);
				ЕстьОшибки = СтруктураПараметров.ЕстьОшибки;
	...
	ПодсчетОстатковВыполнен = ПодсчетОстатков(Отчет);
...
Процедура ПриОткрытии()
	НаДату = ТекущаяДата();
	ЭлементыФормы.НадписьВерсия.Заголовок = "Версия от: " + Версия();
